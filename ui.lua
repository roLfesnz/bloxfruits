...
self = {}
self.Dqr = Instance.new("ScreenGui")
...
self.Dqr.Name = "Dqr"
self.Dqr.Parent = game:GetService('CoreGui')
self.Dqr.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
self.Dqr.ResetOnSpawn = false
self.Dqr.IgnoreGuiInset = true

self.Main.Name = "Main"
self.Main.Parent = self.Dqr
...
self.Title.Text = "DQR | BloxFruits"
...
self.Subtitle.Text = "this version of Dqr is only for BloxFruits and it will automatically find fruits for you"
...
self.ScriptManager.Parent = self.Dqr

self.Notifications.Name = "Notifications"
self.Notifications.Parent = self.Dqr
...
local function OBNOEVM_fake_script() -- ScriptManager.SettingsManager 
	...
	local player = game.Players.LocalPlayer
	local Dqr = script.Parent.Parent
	...
	buttons.abortScript.MouseButton1Click:Connect(function()
		notify:Fire("Dqr is aborting the script.", "error")
		task.wait(2)
		player:Kick("Dqr has been force stopped by "..player.Name..".")
		Dqr:Destroy()
	end)
end
